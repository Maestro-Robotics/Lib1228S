...
lemlib::Chassis::Chassis(Drivetrain_t drivetrain, ChassisController_t lateralSettings,
                         ChassisController_t angularSettings, ChassisController_t swingSettings, OdomSensors_t sensors, DriveCurveFunction_t driveCurve) {
    this->drivetrain = drivetrain;
    this->lateralSettings = lateralSettings;
    this->angularSettings = angularSettings;
    this->swingSettings = swingSettings;
 ...
 * @param log whether the chassis should log the turnTo function. false by default
 */
void lemlib::Chassis::turnToPID(float theta, int timeout, bool async, bool reversed, float maxSpeed, bool log) {
    // try to take the mutex
    // if its unsuccessful after 10ms, return
    if (!mutex.take(10)) return;
    // if the function is async, run it in a new task
    if (async) {
        pros::Task task([&]() { turnToPID(theta, timeout, false, reversed, maxSpeed, log); });
        mutex.give();
        pros::delay(10); // delay to give the task time to start
        return;
    }
 ...
 * @param log whether the chassis should log the turnTo function. false by default
 */
void lemlib::Chassis::swingTo(bool leftSwing, float angle, int timeout, bool async, bool reversed, float maxSpeed, bool log) {
    // try to take the mutex
    // if its unsuccessful after 10ms, return
    if (!mutex.take(10)) return;
    // if the function is async, run it in a new task
    if (async) {
        pros::Task task([&]() { swingTo(leftSwing, angle, timeout, false, reversed, maxSpeed, log); });
        mutex.give();
        pros::delay(10); // delay to give the task time to start
        return;
    }
    ...
    // create a new PID controller
    FAPID pid = FAPID(0, 0, swingSettings.kP, 0, swingSettings.kD, "swingPID");
    pid.setExit(swingSettings.largeError, swingSettings.smallError, swingSettings.largeErrorTimeout,
                swingSettings.smallErrorTimeout, timeout);
    ...
    while (pros::competition::get_status() == compState && !pid.settled()) {
        ...
        if (leftSwing){
            drivetrain.rightMotors->move(-motorPower);
            drivetrain.leftMotors->set_brake_modes(MOTOR_BRAKE_HOLD);
            drivetrain.leftMotors->move(0);
        }
            ...
            drivetrain.leftMotors->move(-motorPower);
            drivetrain.rightMotors->set_brake_modes(MOTOR_BRAKE_HOLD);
            drivetrain.rightMotors->move(0);
        }
    ...
    drivetrain.leftMotors->move(0);
    drivetrain.rightMotors->move(0);
    drivetrain.leftMotors->set_brake_modes(MOTOR_BRAKE_COAST);
    drivetrain.rightMotors->set_brake_modes(MOTOR_BRAKE_COAST);
    // set distTraveled to -1 to indicate that the function has finished
    distTravelled = -1;
    // give the mutex back
    mutex.give();
 ...
 * @param log whether the chassis should log the turnTo function. false by default
 */
void lemlib::Chassis::moveToPID(float x, float y, int timeout, bool async, float maxSpeed, bool log) {
    ...
    // if the function is async, run it in a new task
    if (async) {
        pros::Task task([&]() { moveToPID(x, y, timeout, false, maxSpeed, log); });
        mutex.give();
        pros::delay(10); // delay to give the task time to start
        return;
    }
